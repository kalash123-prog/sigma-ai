...
const constructCommand = (filePath, args) => {
    ...
    const modelPath = args.modelRootPath
        ? path_1.default.resolve(args.modelRootPath, modelName)
        : path_1.default.join(constants_1.WHISPER_CPP_PATH, 'models', modelName);
    if (!fs_1.default.existsSync(modelPath)) {
        errors.push(`[Nodejs-whisper] Error: Model file does not exist at ${modelPath}. Please ensure the model is downloaded and correctly placed.`);
    }
    if (errors.length > 0) {
        throw new Error(errors.join('\n'));
    }
...
const constructOptionsFlags = (args) => {
    ...
    let flags = [
        ...
        ((_o = args.whisperOptions) === null || _o === void 0 ? void 0 : _o.noGpu) ? '-ng ' : '',
    ].join('');
    return flags.trim();
};
